...
class E2024bEstudiantesSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $rutaXlsx = __DIR__ . DIRECTORY_SEPARATOR . 'E2024bEstudiantesDatos.xlsx';
        if (!is_file($rutaXlsx)) {
        ...
        foreach ($datosEstudiantes as $datos) {
            ...
            if (!isset($profesores[$datos['teacher_email']])) {
                $this->command->error("IGNORAR-ESTUDIANTE: Profesor no encontrado con correo: " . $datos['teacher_email']);
                continue;
            }
            ...
            if (!$estudianteEscuela) {
                ...
                $this->command->info("La relación estudiante-escuela se ha creado: EstID $estudianteId, EscID $escuelaId, CatID {$categoria->id} y Grado {$datos['grade']}.");
            } else {
