...
class TestUsersSeeder extends Seeder
{
    ...
    private function createCoordProvincialUser()
    {
        ...
        $this->asignarTerritorioProvincial($user->id, 'Villa Clara');
        
        $this->command->info('Usuario Coordinador Provincial Villa Clara creado: coord.villaclara@bebrascuba.cu / villaclara123');
    }

    private function asignarTerritorioProvincial(int $userId, string $nombreProvincia): void
    {
        $provincia = DB::table('provincias')->where('nombre', $nombreProvincia)->first();
        if (!$provincia) {
            $this->command->warn("Provincia '$nombreProvincia' no encontrada; no se asignó territorio.");
            return;
        }

        $edicion = DB::table('ediciones')->orderByDesc('a_edicion')->first()
            ?? DB::table('ediciones')->orderByDesc('id')->first();

        if (!$edicion) {
            $this->command->warn('No hay ediciones; no se asignó territorio provincial.');
            return;
        }

        $exists = DB::table('coord_regionales')
        ...
        DB::table('coord_regionales')->insert([
            ...
            'municipio_id' => null,
            'descripcion' => "Coordinador provincial {$nombreProvincia}",
    ...
    private function createCoordMunicipalUser()
    {
        ...
        $this->asignarTerritorioMunicipal($user->id, 'Villa Clara', 'Santa Clara');
        
        $this->command->info('Usuario Coordinador Municipal Santa Clara creado: coord.santaclara@bebrascuba.cu / santaclara123');
    }

    private function asignarTerritorioMunicipal(int $userId, string $nombreProvincia, string $nombreMunicipio): void
    {
        $provincia = DB::table('provincias')->where('nombre', $nombreProvincia)->first();
        if (!$provincia) {
            $this->command->warn("Provincia '$nombreProvincia' no encontrada; no se asignó territorio municipal.");
            return;
        }

        $municipio = DB::table('municipios')
            ->where('nombre', $nombreMunicipio)
            ->where('cdgo_provincia', $provincia->codigo)
            ->first();

        if (!$municipio) {
            $this->command->warn("Municipio '$nombreMunicipio' no encontrado; no se asignó territorio municipal.");
            return;
        }
        ...
        if (!$edicion) {
            $this->command->warn('No hay ediciones; no se asignó territorio municipal.');
            return;
        }
        ...
        DB::table('coord_regionales')->insert([
            ...
            'descripcion' => "Coordinador municipal {$nombreMunicipio}",
            'fecha_inicio' => now()->toDateString(),
            'fecha_fin' => null,
            'created_at' => now(),
            'updated_at' => now(),
